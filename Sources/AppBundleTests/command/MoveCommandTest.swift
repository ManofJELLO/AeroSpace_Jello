        ...
        return switch nodeCases {
            ...
            case .tilingContainer(let container):
                switch container.layout {
                    ...
                    case .master:
                        container.orientation == .h
                            ? .h_master(container.children.map(\.layoutDescription))
                            : .v_master(container.children.map(\.layoutDescription))
                }
        }
    }
}
...
enum LayoutDescription: Equatable {
    ...
    case h_master([LayoutDescription])
    case v_master([LayoutDescription])
