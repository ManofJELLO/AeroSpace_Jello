...
* Dragging a window with the mouse puts it where the pointer is: it trades places with whatever window the pointer
  is released over, wherever inside that window -- one pixel into the master area is enough to hand over the master
  slot.
  ...
  This is the same rule `tiles` and `accordion` use, rather than Hyprland's `drop_at_cursor`, which inserts before or
  after the target depending on which half of it the pointer is in.
