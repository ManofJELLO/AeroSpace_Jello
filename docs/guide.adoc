...
. <<layouts,Layout>> (Possible values: `tiles`, `accordion`, `master`)
...
- `h_master` horizontal master (analog of Hyprland’s master layout with `left`/`right`/`center` orientation)
- `v_master` vertical master (analog of Hyprland’s master layout with `top`/`bottom` orientation)
...
`master` is a dynamic layout modelled after
https://wiki.hyprland.org/Configuring/Dwindle-Master-Layouts/#master[Hyprland’s master layout] (and, before that,
after dwm and xmonad). A `master` container splits its children into two groups:

* The *master area* holds the first `count` windows. It takes `fraction` of the container.
* The *stack* holds everything else, and shares whatever the master area leaves behind.

The windows of each group are stacked across the split, so with the master area on the left, master windows are
stacked vertically in the left column and stack windows are stacked vertically in the right column.
...
Unlike `tiles`, the layout is *dynamic*: you never build the tree by hand. Opening a window puts it into the stack
(configurable, see `master.new-window-position`), and the xref:commands.adoc#master[master] command promotes,
demotes, resizes, rotates and reorients from there.

* `focus (left|right|up|down)` moves within a column, and crosses between the master area and the stack.
* `move (left|right|up|down)` swaps the window with its neighbour. Across the split that promotes or demotes it.
* `resize` along the split axis moves the boundary between the master area and the stack.
  Along the other axis it resizes the window within its column.
  Windows share their column evenly until you resize one, and a window that is promoted or demoted keeps its
  proportion rather than dragging a stale size into its new column.
* Dragging a window with the mouse puts it where the pointer is: it takes the slot of whatever window the pointer is
  released over, and the rest shift along by one.
  Anywhere inside that window will do -- one pixel into the master area is enough to hand over the master slot.
  The layout only changes when you release the button, so dragging somewhere and back again cancels the move.
  Set `live-drag-preview = true` to have the windows rearrange as you drag instead.
  That preview is provisional too: it is recomputed from the arrangement the drag started in, so dragging back to
  where you started still restores it.
  It costs a re-layout on every mouse event, which is why it is off by default.
  Position in a `master` container is a window's role -- the leading slots are the master area -- so a drop inserts
  and shifts, where `tiles` and `accordion` swap the two windows instead.
  It differs from Hyprland's `drop_at_cursor` in taking no account of which half of the target the pointer is in.
* A window that refuses to shrink -- Chrome will not go below about 375pt tall -- is given the room it insists on,
  and the windows sharing its column give up the difference.
  Without that it simply overlaps whatever is laid out after it, since an AX size is a request the app is free to
  decline.
  The minimum is learned by reading back what the window actually became after each resize, so it costs no extra
  work and adjusts on its own if the app's minimum changes.
  When the minimums in a column can't all fit, the ordinary proportional split is used: nothing avoids an overlap
  at that point.
* xref:commands.adoc#master[`master`] does everything master-specific: `swap-with-master`, `focus-master`,
  `add-master`, `remove-master`, `set-count`, `set-fraction`, `set-orientation`, `rotate-next`, `rotate-prev`.
...
`center-stack-threshold` and `center-fallback` mirror Hyprland's `slave_count_for_center_master` and
`center_master_fallback`, including that a threshold of `0` centers the master area unconditionally.
When only one of the two flanking columns has a window in it, the other one is left empty so that the master area
stays centered.
Because `center-fallback` names a whole orientation, falling back to `top` or `bottom` flips the split axis, not just
the side.

NOTE: When `default-root-container-layout = 'master'`, the container orientation comes from `master.orientation`
(`left` and `right` are horizontal, `top` and `bottom` are vertical), not from `default-root-container-orientation`.
...
AeroSpace snapshots the workspace's tiling tree at that moment and restores it when the window comes back.
The window returns to the slot and size it left, and its siblings keep theirs.

The snapshot is discarded once a layout-changing command runs, or a window is moved or resized with the mouse.
When that happens, exiting fullscreen falls back to placing the window next to the most recently focused one instead.
Windows that open while the app is fullscreen aren't part of the snapshot, so they get tiled in alongside the restored layout.
Windows that close while the app is fullscreen are simply left out when the layout is restored.
