...
* Dragging a window with the mouse *inserts* it where you drop it, before or after the window under the cursor,
  and shifts the rest along.
  Dropping onto the master area therefore promotes the window to master.
  Dropping onto the near half of a neighbour would resolve to the slot the window is already in; it trades places
  with that neighbour instead, so swapping two stacked windows doesn't require dragging past the far window's
  midpoint.
  ...
  This mirrors Hyprland's `drop_at_cursor`, and differs from `tiles` and `accordion`, where a drag swaps the dragged
  window with the one underneath it.
